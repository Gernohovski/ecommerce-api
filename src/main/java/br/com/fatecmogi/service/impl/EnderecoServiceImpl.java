...
public class EnderecoServiceImpl implements EnderecoService {
	...
	public Endereco salvar(CadastrarEnderecoClienteCommand command) {
		...
		var cliente = clienteRepository.findById(command.getClienteId())
			.orElseThrow(ClienteNaoEncontratoException::new);
		var tipoLogradouro = tipoLogradouroRepository.findById(command.getTipoLogradouroId())
			.orElseThrow(TipoLogradouroNaoEncontratoException::new);
		var tipoResidencia = tipoResidenciaRepository.findById(command.getTipoResidenciaId())
			.orElseThrow(TipoResidenciaNaoEncontratoException::new);
	...
	public Endereco atualizar(Long id, EditarEnderecoClienteCommand command) {
		...
		if (endereco.isPresent()) {
			switch (tipoEndereco) {
				case RESIDENCIAL -> enderecoAtualizado = enderecoMapper
					.updateEnderecoResidencial((EnderecoResidencial) endereco.get(), command);
				case ENTREGA -> enderecoAtualizado = enderecoMapper
					.updateEnderecoEntrega((EnderecoEntrega) endereco.get(), command);
				case COBRANCA -> enderecoAtualizado = enderecoMapper
					.updateEnderecoCobranca((EnderecoCobranca) endereco.get(), command);
			}
		}
