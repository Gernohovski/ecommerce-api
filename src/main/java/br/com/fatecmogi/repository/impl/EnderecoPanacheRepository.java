...
public class EnderecoPanacheRepository implements EnderecoRepository {
	...
	public Endereco save(Endereco endereco) {
		...
		if (endereco instanceof EnderecoResidencial) {
			PanacheEnderecoResidencial panacheEnderecoResidencial = panacheEnderecoMapper
				.fromEnderecoResidencial((EnderecoResidencial) endereco);
			...
			PanacheEnderecoCobranca panacheEnderecoCobranca = panacheEnderecoMapper
				.fromEnderecoCobranca((EnderecoCobranca) endereco);
			...
			PanacheEnderecoEntrega panacheEnderecoEntrega = panacheEnderecoMapper
				.fromEnderecoEntrega((EnderecoEntrega) endereco);
	...
	public Endereco update(Endereco endereco) {
		if (endereco instanceof EnderecoResidencial) {
			PanacheEnderecoResidencial panacheEnderecoResidencialSalvo = PanacheEnderecoResidencial
				.findById(endereco.getId());
			panacheEnderecoResidencialSalvo.unreferrenceDependecies();
			PanacheEnderecoResidencial panacheEnderecoResidencial = panacheEnderecoMapper
				.updateEnderecoResidencial(panacheEnderecoResidencialSalvo, endereco);
			panacheEnderecoResidencial.persist();
			return panacheEnderecoMapper.from(panacheEnderecoResidencial);
		}
		else if (endereco instanceof EnderecoCobranca) {
			...
			PanacheEnderecoCobranca panacheEnderecoCobranca = panacheEnderecoMapper
				.updateEnderecoCobranca(panacheEnderecoCobrancaSalvo, endereco);
			panacheEnderecoCobranca.persist();
			return panacheEnderecoMapper.from(panacheEnderecoCobranca);
		}
		else {
			...
			PanacheEnderecoEntrega panacheEnderecoEntrega = panacheEnderecoMapper
				.updateEnderecoEntrega(panacheEnderecoEntregaSalvo, endereco);
			panacheEnderecoEntrega.persist();
			return panacheEnderecoMapper.from(panacheEnderecoEntrega);
		}
	}
	...
	public Optional<Endereco> findById(Long id, TipoEndereco tipoEndereco) {
		...
		switch (tipoEndereco) {
			case RESIDENCIAL -> endereco = panacheEnderecoMapper
				.from((PanacheEnderecoResidencial) PanacheEnderecoResidencial.findById(id));
			case COBRANCA ->
				endereco = panacheEnderecoMapper.from((PanacheEnderecoCobranca) PanacheEnderecoCobranca.findById(id));
			case ENTREGA ->
				endereco = panacheEnderecoMapper.from((PanacheEnderecoEntrega) PanacheEnderecoEntrega.findById(id));
		}
		if (endereco == null) {
	...
	public List<Endereco> findAllByCliente(Long clienteId, TipoEndereco tipoEndereco) {
		...
		switch (tipoEndereco) {
			case RESIDENCIAL -> {
				List<PanacheEnderecoResidencial> enderecosResidenciais = PanacheEnderecoResidencial
					.find("cliente.id", clienteId)
					.list();
				enderecos = panacheEnderecoMapper.fromEnderecoResidencial(enderecosResidenciais);
			}
			case COBRANCA -> {
				List<PanacheEnderecoCobranca> enderecoCobrancas = PanacheEnderecoCobranca.find("cliente.id", clienteId)
					.list();
				enderecos = panacheEnderecoMapper.fromEnderecoCobranca(enderecoCobrancas);
			}
			case ENTREGA -> {
				List<PanacheEnderecoEntrega> enderecoEntregas = PanacheEnderecoEntrega.find("cliente.id", clienteId)
					.list();
